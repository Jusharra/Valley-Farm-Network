...
create table public.delivery_schedules (
  ...
  time_window text not null,   -- e.g. '9am–12pm'
  is_active   boolean not null default true,
  created_at  timestamptz not null default now()
);
...
-- Atomic job acceptance — first driver to call this wins.
-- Assigns the driver to the delivery_jobs row and the deliveries row.
